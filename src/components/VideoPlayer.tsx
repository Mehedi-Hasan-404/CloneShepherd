// /src/components/VideoPlayer.tsx - Merged and Improved Version
...
const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  const detectStreamType = useCallback((url: string): { type: 'hls' | 'dash' | 'native'; cleanUrl: string; drmInfo?: any } => {
    ...
    // Fallback for URLs without extensions (common in some IPTV setups)
    return { type: 'hls', cleanUrl, drmInfo };
  }, []);
  ...
  const initHlsPlayer = async (url: string, video: HTMLVideoElement) => {
    try {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, (_, data) => {
        ...
        setPlayerState(prev => ({ ...prev, isLoading: false, error: null, availableQualities: levels, currentQuality: hls.currentLevel, isMuted: video.muted, isPlaying: !video.paused, isLive: data.details.live, duration: data.details.live ? 0 : video.duration }));
      });
  ...
  const initShakaPlayer = async (url: string, video: HTMLVideoElement, drmInfo?: any) => {
    try {
        ...
        setPlayerState(prev => ({ ...prev, isLoading: false, error: null, availableQualities: qualities, availableSubtitles: subtitles, isMuted: video.muted, isPlaying: !video.paused, isLive, duration: isLive ? 0 : video.duration }));
    } catch (error) {
        if (loadingTimeoutRef.current) clearTimeout(loadingTimeoutRef.current);
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      setPlayerState(prev => ({ ...prev, currentTime: video.currentTime, buffered: video.buffered.length > 0 ? video.buffered.end(video.buffered.length - 1) : 0, duration: playerState.isLive ? 0 : video.duration }));
    };
  ...
  const calculateNewTime = useCallback((clientX: number): number | null => {
    ...
    const video = videoRef.current;
    if (!progressBar || !video || playerState.isLive || !isFinite(video.duration)) return null;
    ...
    >
      ...
      {playerState.isLoading && (
        <div className="absolute inset-0 bg-black/70 flex items-center justify-center z-20">
            <div className="text-center text-white"><Loader2 className="w-10 h-10 mx-auto mb-3 animate-spin" /><p>Loading Stream...</p></div>
        </div>
      )}
      ...
      {playerState.error && (
        <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-20 text-center p-4">
            <div>
                ...
                <button onClick={handleRetry} className="inline-flex items-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg">
      ...
      <div className={`controls-area absolute inset-0 transition-opacity duration-300 z-10 ${playerState.showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={(e) => e.stopPropagation()}>
        ...
        {!playerState.isPlaying && !playerState.isLoading && !playerState.error && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <button onClick={togglePlay} className="pointer-events-auto w-20 h-20 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-all scale-100 hover:scale-110">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 space-y-2">
          {isSeekable && (
            <div ref={progressRef} className="relative h-1.5 bg-white/20 rounded-full group/seekbar cursor-pointer" onClick={handleProgressClick}>
              ...
              <div className="absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 rounded-full bg-red-500 transition-transform duration-150 ease-out group-hover/seekbar:scale-125" style={{ left: `${currentTimePercentage}%` }} onMouseDown={handleSeekStart} onClick={(e) => e.stopPropagation()}/>
            </div>
          )}
          <div className="flex items-center gap-4 text-white">
            <button onClick={togglePlay} className="hover:scale-110 transition-transform">{playerState.isPlaying ? <Pause size={24} /> : <Play size={24} />}</button>
            <button onClick={toggleMute} className="hover:scale-110 transition-transform">{playerState.isMuted ? <VolumeX size={24} /> : <Volume2 size={24} />}</button>
            {playerState.isLive ? (<div className="flex items-center gap-2"><div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div><span className="text-xs font-semibold uppercase">LIVE</span></div>) : (<div className="text-sm font-mono">{formatTime(playerState.currentTime)} / {formatTime(playerState.duration)}</div>)}
            <div className="flex-grow"></div>
            {document.pictureInPictureEnabled && (<button onClick={togglePip} className="hover:scale-110 transition-transform" title="Picture-in-picture"><PictureInPicture2 size={20} /></button>)}
            <button onClick={toggleFullscreen} className="hover:scale-110 transition-transform" title="Fullscreen">{playerState.isFullscreen ? <Minimize size={22} /> : <Maximize size={22} />}</button>
          </div>
        </div>
      </div>
      <Drawer open={playerState.showSettings} onOpenChange={(isOpen) => setPlayerState(p => ({ ...p, showSettings: isOpen }))}>
        <DrawerContent className="bg-[#0a0a0a] text-white border-t border-gray-700 outline-none landscape:max-w-md landscape:mx-auto">
          <DrawerHeader><DrawerTitle className="text-center text-xl">Stream Settings</DrawerTitle></DrawerHeader>
          ...
          <Accordion type="single" collapsible className="w-full">
                ...
                <AccordionItem value="quality" className="border-b border-gray-700">
                  ...
                  <AccordionContent>
                    <div className="space-y-1 pt-2">
                      <button onClick={() => changeQuality(-1)} className={`w-full text-left px-3 py-2 rounded transition-colors ${playerState.currentQuality === -1 ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-white/10'}`}>Auto</button>
                      {playerState.availableQualities.map((quality) => (
                        <button key={quality.id} onClick={() => changeQuality(quality.id)} className={`w-full text-left px-3 py-2 rounded transition-colors ${playerState.currentQuality === quality.id ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-white/10'}`}>
              ...
              {playerState.availableSubtitles.length > 0 && (
                <AccordionItem value="subtitles" className="border-b border-gray-700">
                  ...
                  <AccordionContent>
                    <div className="space-y-1 pt-2">
                      <button onClick={() => changeSubtitle('')} className={`w-full text-left px-3 py-2 rounded transition-colors ${playerState.currentSubtitle === '' ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-white/10'}`}>Off</button>
                      {playerState.availableSubtitles.map((subtitle) => (
                        <button key={subtitle.id} onClick={() => changeSubtitle(subtitle.id)} className={`w-full text-left px-3 py-2 rounded transition-colors ${playerState.currentSubtitle === subtitle.id ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-white/10'}`}>{subtitle.label}</button>
                      ))}
                    </div>
                  </AccordionContent>
                </AccordionItem>
                ...
                <AccordionContent>
                  <div className="space-y-1 pt-2">
                    {[0.5, 0.75, 1, 1.25, 1.5, 2].map(speed => (
                      <button key={speed} onClick={() => { if (videoRef.current) { videoRef.current.playbackRate = speed; } setPlayerState(prev => ({ ...prev, showSettings: false })); }} className={`w-full text-left px-3 py-2 rounded transition-colors ${videoRef.current?.playbackRate === speed ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-white/10'}`}>
