...
import { ArrowLeft, Star, Share2, AlertCircle, Search, Play } from 
'lucide-react';
...
const cleanChannelName = currentChannel.name.replace(/[^a-zA-Z0-
9]/g, '_').toLowerCase();
...
console.error(`Error loading M3U playlist for category $
{categoryData.name}:`, m3uError);
}
}
}
...
if (!foundChannel) {
setError('Channel not found. The channel may have been removed or 
the link is invalid.');
return;
}
...
if (error || !channel) {
const displayError = error || 'Channel not found or stream is 
missing.';
return (
<div className="space-y-6 p-4 sm:p-6">
<Button 
...
{isChannelFavorite ? 'text-yellow-500 border-yellow-500/50 bg-yellow-500/10' : ''}`}
>
<Star 
size={16} 
fill={isChannelFavorite ? 'currentColor' : 'none'} 
className={`mr-1 transition-all duration-300 $
{isChannelFavorite ? 'animate-pulse' : ''}`} 
/>
...
alt={channel.name}
className="w-16 h-16 object-contain p-1 bg-white dark:bg-
...
<Badge variant="destructive" className="animate-
pulse">LIVE</Badge>
</div>
</div>
</div>
...
More Channels
{channel.categoryName && <span className="text-base text-
gray-500 font-normal ml-2">in {channel.categoryName}</span>}
</h2>
...
text-card-foreground focus:ring-2 focus:ring-accent focus:border-accent shadow-
inner"
/>
</div>
...
<div 
key={ch.id} 
className="channel-card cursor-pointer p-3 border 
rounded-lg hover:border-accent transition-all duration-300 bg-card shadow-sm 
...
z-10 transition-all duration-300 transform hover:scale-110 ${
isRelatedFavorite
? 'bg-yellow-500 text-white shadow-lg animate-
bounce'
: 'bg-black/50 text-white hover:bg-black/70'
}`}
title={isRelatedFavorite ? 'Remove from favorites' :
'Add to favorites'}
>
<Star 
size={12} 
...
alt={ch.name}
className="w-full h-12 sm:h-16 object-contain mb-2 
p-1"
